...
str_sim = (np.expand_dims(TUsketch_label_after, axis=1) == np.expand_dims(TUimage_label_after, axis=0)) * 1

distance1 = distance1 = cdist(TUsketch_test, TUimage_test, 'euclidean')
sim1 = 1/(1+distance1)
...
for j in range(n_target-1,n_target):
    for i in range(n_class):
        ...
        TUimage_k=TUimage_test[(TUimage_label_after==TU_test_class[i])][3:j+3]
        ...
        if (matrix_rank(TUimage_k)!=TUimage_k.shape[0]):
            TUimage_k=delete_same_image(TUimage_k)
        ...
        # target=np.dot(target,Q)
        # target=np.dot(target,omega)
        # target=np.dot(target,np.linalg.pinv(Pt))
    
        # target_i[i]=target*zi_norm
        # target_i[i]=target
        target=target/np.linalg.norm(target)
        _,target_norm=normalize(TUimage_k,return_norm=True)
        target_norm=np.mean(target_norm)
        # target=target*target_norm
        target_i[i]=target
        
        
        T_mean_GT=np.mean(TUimage_k,axis=0)
        T_mean_GT=T_mean_GT/np.linalg.norm(T_mean_GT)
        T_mean_GT_all=np.mean(TUimage_test[(TUimage_label_after==TU_test_class[i])],axis=0)
        
        T_mean_GT_all=T_mean_GT_all/np.linalg.norm(T_mean_GT_all)
        target_GT_i[i]=T_mean_GT_all
        
        
        
    
        
        dist_mean_i[i]=np.linalg.norm(target_i[i]-T_mean_GT)
        dist_mean_all_i[i]=np.linalg.norm(target_i[i]-T_mean_GT_all)
        
        sketch_k=sketch_test_after[(sketch_label_after==sketch_test_class[i])]
        TUsketch_k=TUsketch_test[(TUsketch_label_after==TU_test_class[i])][0:j]
        # TUsketch_k_all=TUsketch[(label2==i)]
        # TUsketch_k_normal=TUsketch_k/np.linalg.norm(TUsketch_k)
        ...
        # if matrix_rank(sketch2!=sketch2.shape[0]):
        #     sketch2=delete_same_image(sketch2)
        if (matrix_rank(TUsketch_k)!=TUsketch_k.shape[0]):
            TUsketch_k=delete_same_image(TUsketch_k)
        
        
        zi_after=sketch_k.copy()
        ti=TUsketch_k.copy()
        ...
        # target_i[i]=target
        target=target/np.linalg.norm(target)
        _,target_norm=normalize(TUsketch_k,return_norm=True)
        target_norm=np.mean(target_norm)
        # target=target*target_norm
        target_s[i]=target
        ...
        T_mean_GT=T_mean_GT/np.linalg.norm(T_mean_GT)
        T_mean_GT_all=np.mean(TUsketch_test[(TUsketch_label_after==TU_test_class[i])],axis=0)
        
        T_mean_GT_all=T_mean_GT_all/np.linalg.norm(T_mean_GT_all)
        target_GT_s[i]=T_mean_GT_all
    
        
        dist_mean_s[i]=np.linalg.norm(target_i[i]-T_mean_GT)
        dist_mean_all_s[i]=np.linalg.norm(target_i[i]-T_mean_GT_all)
    
    str_sim = (np.expand_dims(TUsketch_label_after, axis=1) == np.expand_dims(TUimage_label_after, axis=0)) * 1
    # str_sim = str_sim[(sketchlabeltest==list(set(imagelabeltest))[class_index])]
    
    
    distance_i = cdist(target_i,normalize(TUimage_test), 'euclidean')
    distance_s = cdist(target_s,normalize(TUsketch_test), 'euclidean')
    ...
    for i in range(retrival_sketch.shape[0]):
        sketch_class[i]=TU_test_class[retrival_sketch[i][0]]
        
    for i in range(retrival_image_2.shape[0]):
        image_class[i]=TU_test_class[retrival_image_2[i][0]]
        
    accuracy_s=(sketch_class==TUsketch_label_after.reshape(-1,1)).sum()/sketch_class.shape[0]
    accuracy_i=(image_class==TUimage_label_after.reshape(-1,1)).sum()/image_class.shape[0]
    ...
    for i in range(n_class):
        s_1=(sketch_class[TUsketch_label_after==TU_test_class[i]])
        s_2=(TUsketch_label_after[TUsketch_label_after==TU_test_class[i]]).reshape(-1,1)
        accuracy_class_s[i]=(s_1==s_2).sum()/s_1.shape[0]
        
    accuracy_class_i = np.zeros((n_class,1))
    for i in range(n_class):
        s_1=(image_class[TUimage_label_after==TU_test_class[i]])
        s_2=(TUimage_label_after[TUimage_label_after==TU_test_class[i]]).reshape(-1,1)
...
# 		# 在图中为每个数据点画出标签
# 		plt.text(data[i, 0], data[i, 1], str(label[i]), color=plt.cm.Set1(label[i] / 122),
...
# result = np.append(normalize(TUsketch_test),target_s,axis=0)
...
# # class_index_plot=np.argmax(dist_mean_all_s)

# result=ts_result[0:ts_result.shape[0]-target_s.shape[0]*2][TUsketch_label_after==TU_test_class[class_index_plot]]
# fig = plt.figure()
...
result = np.append(normalize(TUimage_test),target_i,axis=0)
result = np.append(result,target_GT_i,axis=0)
# result = ts.fit_transform(result)
...
# class_index_plot=np.argmin(accuracy_class_i)
class_index_plot=np.argmax(accuracy_class_i)

# class_index_plot=np.argmin(dist_mean_all_i)
# class_index_plot=np.argmax(dist_mean_all_i)

result=ts_result[0:ts_result.shape[0]-target_s.shape[0]*2][TUimage_label_after==TU_test_class[class_index_plot]]
